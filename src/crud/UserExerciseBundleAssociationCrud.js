...
async function createUserExerciseBundleAssociation(user_id,bundle_id) {
    await pool.query(
        'INSERT INTO speech_therapy.user_bundle (user_id,bundle_id) VALUES ($1, $2)',
        [user_id,bundle_id]
    );
    return new UserExerciseBundleAssociation(user_id,bundle_id);
}
...
async function getUserExerciseBundleAssociationByUserId(user_id) {
    ...
    const userBundleRes = await pool.query('SELECT bundle_id FROM speech_therapy.user_bundle WHERE user_id = $1', [user_id]);
